...
DECLARE C_EXISTS INTEGER;
BEGIN
	SELECT INDEX_EXISTS(C_TABLE_SCHEMA, C_TABLE_NAME, lower('idx_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_'))) INTO C_EXISTS;
	IF C_EXISTS = 0
	THEN
		EXECUTE 'CREATE INDEX '||lower('idx_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_'))||' ON '||C_TABLE_NAME||'('||C_COLUMN_NAME||')';
	END IF;
END 
$$
LANGUAGE plpgsql;
...
BEGIN
	...
	IF C_EXISTS > 0
	THEN
		EXECUTE 'DROP INDEX '||lower('idx_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_'));
	END IF;
END 
$$
LANGUAGE plpgsql;
...
$$
BEGIN
	PERFORM DML_CONSTRAINT(C_TABLE_SCHEMA, C_TABLE_NAME, 'pk_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_'), 'PRIMARY KEY('||C_COLUMN_NAME||')');
END 
$$
LANGUAGE plpgsql;
...
$$
BEGIN
	PERFORM DROP_CONSTRAINT(C_TABLE_SCHEMA, C_TABLE_NAME, 'pk_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_')); 
END 
$$
LANGUAGE plpgsql;
...
$$
BEGIN
	PERFORM DML_CONSTRAINT(C_TABLE_SCHEMA, C_TABLE_NAME, 'uk_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_'), 'UNIQUE('||C_COLUMN_NAME||')');
END 
$$
LANGUAGE plpgsql;
...
$$
BEGIN
	PERFORM DROP_CONSTRAINT(C_TABLE_SCHEMA, C_TABLE_NAME, 'uk_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_')); 
END 
$$
LANGUAGE plpgsql;
...
BEGIN
	PERFORM DML_CONSTRAINT(
		C_TABLE_SCHEMA, 
		C_TABLE_NAME, 
		'fk_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_'), 
...
$$
BEGIN
	PERFORM DROP_CONSTRAINT(C_TABLE_SCHEMA, C_TABLE_NAME, 'fk_'||C_TABLE_NAME||'_'||REPLACE(C_COLUMN_NAME, ',', '_')); 
END 
$$
LANGUAGE plpgsql;
...
-- CREATE OR REPLACE FUNCTION  update_y0struct_partition() RETURNS TRIGGER AS
-- $$
-- DECLARE C_PART_TABLE_NAME VARCHAR(64);
-- DECLARE C_PART_BEGIN VARCHAR(64);
-- DECLARE C_PART_END VARCHAR(64);
-- DECLARE C_EXISTS INTEGER;
-- DECLARE C_COL_VALUES TEXT;
-- DECLARE COL_NAME TEXT;
-- BEGIN
-- 	C_PART_TABLE_NAME := lower(TG_TABLE_NAME||'_'||to_char(old.CREATE_TIME, 'YYYY'));
-- 	SELECT 
-- 		count(1) INTO C_EXISTS
-- 	FROM 
-- 		INFORMATION_SCHEMA.TABLES
...
-- 	SELECT '' INTO C_COL_VALUES;
-- 	FOR COL_NAME IN SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = C_PART_TABLE_NAME AND table_schema = TG_TABLE_SCHEMA LOOP
--         IF NEW.COL_NAME IS DISTINCT FROM OLD.COL_NAME THEN
-- 			IF C_EXISTS = ''
-- 			THEN
-- 				SELECT C_COL_VALUES||' '||COL_NAME||' = ($1).'||COL_NAME INTO C_COL_VALUES;
-- 			ELSE
-- 				SELECT C_COL_VALUES||' ,'||COL_NAME||' = ($1).'||COL_NAME INTO C_COL_VALUES;
-- 			END IF;
--         END IF;
...
-- 	EXECUTE 'UPDATE '||C_PART_TABLE_NAME||' SET '||C_COL_VALUES||' WHERE id = ($1).id' USING NEW;
-- 	return NULL;
-- END
-- $$
-- LANGUAGE plpgsql;

-- CREATE OR REPLACE FUNCTION  update_m0struct_partition() RETURNS TRIGGER AS
-- $$
-- DECLARE C_PART_TABLE_NAME VARCHAR(64);
-- DECLARE C_PART_BEGIN VARCHAR(64);
-- DECLARE C_PART_END VARCHAR(64);
...
-- CREATE OR REPLACE FUNCTION  update_d0struct_partition() RETURNS TRIGGER AS
-- $$
-- DECLARE C_PART_TABLE_NAME VARCHAR(64);
-- DECLARE C_PART_BEGIN VARCHAR(64);
-- DECLARE C_PART_END VARCHAR(64);
...
DROP FUNCTION IF EXISTS DML_CUSTOM_PART;
CREATE FUNCTION DML_CUSTOM_PART(
	IN C_TABLE_SCHEMA VARCHAR(32),
	IN C_TABLE_NAME VARCHAR(64),
	IN C_PART_TRIGGER_NAME VARCHAR(64)
) RETURNS "pg_catalog"."void" AS 
$$
BEGIN
	PERFORM DML_TRIGGER(C_TABLE_SCHEMA, C_TABLE_NAME, 'BEFORE', 'INSERT', C_TABLE_NAME||'_ctm0stp', 'EXECUTE PROCEDURE '||C_PART_TRIGGER_NAME||'()');
END 
$$
LANGUAGE plpgsql;
